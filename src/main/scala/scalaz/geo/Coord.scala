...
object Coord {
  ...
  implicit val coordInstances = new scalaz.Show[Coord] with scalaz.Order[Coord] {
    override def shows(c: Coord) = "[" + scalaz.Show[Latitude].shows(c.latitude) + " " + scalaz.Show[Longitude].shows(c.longitude) + "]"
    def order(c1: Coord, c2: Coord) =
      scalaz.Order[(Latitude, Longitude)].order((c1.latitude, c1.longitude), (c2.latitude, c2.longitude))
  }
}
