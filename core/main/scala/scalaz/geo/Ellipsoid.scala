...
object Ellipsoid {
  implicit val ellipsoidInstances = new scalaz.Show[Ellipsoid] with scalaz.Order[Ellipsoid] {
    override def shows(e: Ellipsoid) = (e.semiMajor, e.semiMinor, e.flattening, e.inverseFlattening).toString
    def order(e1: Ellipsoid, e2: Ellipsoid) =
      scalaz.Order[(Double, Double, Double, Double)].order(
        (e1.semiMajor, e1.semiMinor, e1.flattening, e1.inverseFlattening),
        (e2.semiMajor, e2.semiMinor, e2.flattening, e2.inverseFlattening))
  }
}
